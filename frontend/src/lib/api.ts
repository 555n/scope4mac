...
export interface TempoSourcesResponse {
  sources: Record<string, { available: boolean; name: string; devices?: string[]; install_hint?: string }>;
}
...
export const getTempoStatus = async (): Promise<TempoStatusResponse> => {
  ...
  if (!response.ok) throw new Error(`Failed to get tempo status: ${response.statusText}`);
  return response.json();
};

export const enableTempo = async (request: TempoEnableRequest): Promise<TempoStatusResponse> => {
  const response = await fetch("/api/v1/tempo/enable", {
    method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(request),
  });
  if (!response.ok) throw new Error(`Failed to enable tempo: ${await response.text()}`);
  return response.json();
};

export const disableTempo = async (): Promise<TempoStatusResponse> => {
  const response = await fetch("/api/v1/tempo/disable", { method: "POST" });
  if (!response.ok) throw new Error(`Failed to disable tempo: ${response.statusText}`);
  return response.json();
};

export const setTempo = async (bpm: number): Promise<TempoStatusResponse> => {
  const response = await fetch("/api/v1/tempo/set_tempo", {
    method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ bpm }),
  });
  if (!response.ok) throw new Error(`Failed to set tempo: ${await response.text()}`);
  return response.json();
};

export const getTempoSources = async (): Promise<TempoSourcesResponse> => {
  const response = await fetch("/api/v1/tempo/sources");
  if (!response.ok) throw new Error(`Failed to get tempo sources: ${response.statusText}`);
  return response.json();
};
