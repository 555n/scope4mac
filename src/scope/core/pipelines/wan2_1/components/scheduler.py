...
class SchedulerInterface(ABC):
    ...
    def convert_x0_to_noise(
        ...
        x0, xt, alphas_cumprod = (
            x.float().to(x0.device) for x in [x0, xt, self.alphas_cumprod]
        )
    ...
    def convert_noise_to_x0(
        ...
        noise, xt, alphas_cumprod = (
            x.float().to(noise.device) for x in [noise, xt, self.alphas_cumprod]
        )
        alpha_prod_t = alphas_cumprod[timestep].reshape(-1, 1, 1, 1)
        beta_prod_t = 1 - alpha_prod_t
    ...
    def convert_velocity_to_x0(
        ...
        velocity, xt, alphas_cumprod = (
            x.float().to(velocity.device) for x in [velocity, xt, self.alphas_cumprod]
        )
        alpha_prod_t = alphas_cumprod[timestep].reshape(-1, 1, 1, 1)
        beta_prod_t = 1 - alpha_prod_t
