...
class MemFlowPipeline(Pipeline, LoRAEnabledPipeline, VACEEnabledPipeline):
    ...
    def __init__(
        ...
        # Strategy: Load MemFlow base, wrap with VACE (if enabled), add VACE weights, then apply LoRA
        ...
        # Apply MemFlow's built-in performance LoRA using the module-targeted strategy.
        # This mirrors the original MemFlow behavior and is independent of any
        # additional runtime LoRA strategies managed by LoRAEnabledPipeline.
        if lora_path is not None:
            start = time.time()
            # MemFlow's adapter config is passed through unchanged so the
            # module-targeted manager can construct the PEFT config exactly
            # like the original implementation.
            memflow_lora_config = dict(lora_config) if lora_config is not None else {}
            generator.model = ModuleTargetedLoRAStrategy._configure_lora_for_model(
                generator.model,
                model_name=generator_model_name,
                lora_config=memflow_lora_config,
            )
